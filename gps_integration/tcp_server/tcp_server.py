...
PORT = 6824  #change this to your port
...
def start_server_trigger():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            conn, addr = s.accept()
            conn.settimeout(700) #connection timeout, change this value to close the socket if no DATA is received for X amount of seconds
            with conn:
...
def codec_8e_parser(codec_8E_packet, device_imei, props):
    ...
    while data_field_position < (2*data_field_length - 6):                
        ...
        for i in range(byte1_io_number_parsed):
            ...
            if key_int == 239:
                io_dict["ignition_status"] = "ON" if int(value, 16) == 1 else "OFF"
        ...
        for i in range(byte4_io_number_parsed):
            ...
            if key_int == 16:  # Odometer ID
                io_dict["odometer"] = value_int
            io_dict[int(key, 16)] = sorting_hat(int(key, 16), value)
            data_field_position += len(value)
        ...
        if codec_type.upper() == "8E":
            ...
            for i in range(byteX_io_number_parsed):
                ...
                io_dict[int(key, 16)] = sorting_hat(int(key, 16), value)        
                data_field_position += len(value)
        trip_status = io_dict.get("trip_status", "Unknown")
        if trip_status == "In Trip":
            trip_route.append({
                "timestamp": io_dict["_timestamp_"],
        ...
        if latest_record:
            try:
                ...
                gps_log = frappe.get_doc({
                    ...
                    'received_at': latest_record['_timestamp_'],
                    ...
                    'ignition_status': latest_record.get('ignition_status', 'UNKNOWN'),
                    'odometer': latest_record.get('odometer', 0),
                ...
                frappe.db.commit()
                apiKey = "6ad2f1391c757b1"      # Replace with your API Key
                apiSecret = "4233825aaead1f7" # Replace with your API Secret
                url = "https://delivery.thelegendsoft.com/api/resource/gps_log"  # Replace with your endpoint URL
                ...
                headers = {
                    "Authorization": f"token {apiKey}:{apiSecret}",
                    "Content-Type": "application/json"
                }

                # Send the POST request to the remote ERPNext server
                try:
                    response = requests.post(url, data=json_data, headers=headers)

                    # Check if the request was successful
                    if response.status_code == 200:
                        frappe.msgprint("GPS Log created successfully in the remote ERPNext instance!")
                        return response.json()
                    else:
                        frappe.throw(f"Failed to create GPS Log. Status Code: {response.status_code}, Response: {response.text}")

                except Exception as e:
                    frappe.log_error(frappe.get_traceback(), 'Error sending GPS Log to remote server')
                    frappe.throw(f"Error occurred while sending GPS Log: {str(e)}")

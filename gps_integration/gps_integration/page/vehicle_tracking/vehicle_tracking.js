frappe.pages['vehicle-tracking'].on_page_load = function(wrapper) {
    ...
    function fetchLatestGPSLogs(timeRange = 'all') {
        ...
        return frappe.call({
            ...
            args: {
                doctype: 'GPS Log',
                fields: ['name', 'latitude', 'longitude', 'device_id', 'vehicle', 'modified', 'speed', 'heading'],
                filters: filters,
            ...
            r.message.forEach(log => {
                if (!latestLogs[log.device_id] || new Date(log.modified) > new Date(latestLogs[log.device_id].modified)) {
                    latestLogs[log.device_id] = log;
                }
            });
        ...
        let vehicleListHTML = logs.map(log => {
            ...
            return `
                <div class="vehicle-item" data-device-id="${log.device_id}">
                    ...
                    <small>Last updated: ${log.modified}</small>
                </div>
            `;
        }).join('');
    ...
    function updateMarkers(logs) {
        ...
        let geocoder = new google.maps.Geocoder();  // Initialize geocoder

        logs.forEach(log => {
            let position = new google.maps.LatLng(log.latitude, log.longitude);
            let markerIcon = {
                path: "M 0,0 L -10,-10 L 0,-5 L 10,-10 L 0,0",  // SVG-like path for an arrow
                fillColor: getStatusColor(log.speed),
                fillOpacity: 0.8,
                scale: 2,  // Adjust the size of the arrow
                strokeColor: 'white',
                strokeWeight: 2,
                rotation: log.heading || 0  // Rotate arrow based on heading or direction
            };
                ...
                geocoder.geocode({ 'location': position }, (results, status) => {
                    if (status === 'OK') {
                        if (results[0]) {
                            ...
                            // Add info window with location details, including address

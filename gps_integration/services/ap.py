...
def start_server_trigger():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            conn, addr = s.accept()
            print("Test")
            conn.settimeout(5) #connection timeout, change this value to close the socket if no DATA is received for X amount of seconds
            print("Test")
            with conn:
...
def codec_8e_parser(codec_8E_packet, device_imei, props):
    ...
    while data_field_position < (2*data_field_length - 6):                
        ...
        for i in range(byte1_io_number_parsed):
            ...
            key_int = int(key, 16)
            io_dict[key_int] = sorting_hat(key_int, value)
            data_field_position += len(value)

            # Check for ignition status (ID 239)
            if key_int == 239:
    ...
    if records:
        latest_record = max(records, key=lambda r: datetime.datetime.strptime(extract_timestamp(r["_timestamp_"]), "%H:%M:%S %d-%m-%Y"))
        ...
        if latest_record:
            try:
                ...
                gps_log = frappe.get_doc({
                    ...
                    'ignition_status': latest_record.get('ignition_status', 'UNKNOWN'),  # Add ignition status to GPS Log
